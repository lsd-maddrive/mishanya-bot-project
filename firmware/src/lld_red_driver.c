 ...
 */
void lld_red_driver_direct(const control_driver_t *control,
                           const pwm_channel_t *pwm_ch, uint16_t period) {
 ...
 * @brief recieve control struct, pwm channel and and the filling period
 */
void lld_red_driver_reverse(const control_driver_t *control,
                            const pwm_channel_t *pwm_ch, uint16_t period) {
 ...
 */
void lld_red_driver_off(const control_driver_t *control,
                        const pwm_channel_t *pwm_ch) {
  palWriteLine(control->line_control.digit_1, PAL_LOW);
  palWriteLine(control->line_control.digit_2, PAL_LOW);
